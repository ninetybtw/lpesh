...
// (Минцифры), которого нет в стандартном доверенном хранилище — попытка
// довериться ему через переменную окружения DENO_CERT не сработала (этот
// edge-runtime, судя по всему, её не читает), поэтому сертификат зашит
// прямо здесь и передаётся через Deno.createHttpClient({ caCerts }) —
// это работает на уровне кода конкретного fetch-запроса, а не окружения.
...
const gigaHttpClient = Deno.createHttpClient({
