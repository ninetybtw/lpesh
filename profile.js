...
const PLAN_PRICES = {
  basic: 'Бесплатно',
  pro: '600 ₽/мес (при годовой оплате — 420 ₽/мес, 5 040 ₽/год)',
  max: '1000 ₽/мес (при годовой оплате — 800 ₽/мес, 9 600 ₽/год)'
};
// Набор пунктов зависит от периода оплаты — см. LexPrepPlan.hasAnnualPlan()
// и цифры в price-card на index.html#pricing (та же логика, что и лимиты
// в plan.js: годовая оплата даёт больше запросов ИИ, дуэлей и турниров).
const PLAN_FEATURES = {
  ...
  pro: {
    monthly: ['Все конспекты и карточки без ограничений', '3 попытки теста в день с разбором', 'Пробный экзамен — 3 попытки в месяц', 'ИИ-консультант — 5 запросов в день, проверка эссе — 2/мес', 'Дуэли — 3/день, турниры — 1/мес без взноса', 'Публикация своих тестов и статей (после модерации)', 'Без рекламы'],
    annual: ['Все конспекты и карточки без ограничений', '5 попыток теста в день с разбором', 'Пробный экзамен — 3 попытки в месяц', 'Продвинутый ИИ-консультант — 10 запросов в день', 'Дуэли — 5/день, турниры — 3/мес без взноса', 'Публикация своих тестов и статей (после модерации)', 'Без рекламы']
  },
  max: {
    monthly: ['Всё из «Про» без лимитов', 'ИИ-консультант — 25–40 запросов в день', 'Безлимит: пробные экзамены, дуэли', 'Турниры — до 5 участий в месяц без взноса', 'Экспорт конспектов в PDF'],
    annual: ['Всё из «Про» без лимитов', 'Продвинутый ИИ-консультант — 40–60 запросов в день', 'Безлимит: пробные экзамены, дуэли', 'Турниры — до 10 участий в месяц без взноса', 'Экспорт конспектов в PDF']
  }
};
