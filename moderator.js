...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const feedbackList = document.getElementById('modFeedbackList');
  const feedbackRefreshBtn = document.getElementById('modFeedbackRefreshBtn');
  const feedbackNewCountEl = document.getElementById('modFeedbackNewCount');
    ...
    feedbackList.innerHTML = items.map(f => `
      <div class="community-item" data-feedback-id="${f.id}">
        <div class="community-item__head">
          ...
          <span class="community-badge community-badge--${f.status}">${MOD_FEEDBACK_STATUS_LABEL[f.status] || f.status}</span>
        </div>
